...
| Experiment | Input res | Spatial F1 @10px | Temporal Macro-F1 @2f | Temporal Micro-F1 @2f |
|---|---|---|---|---|
| E1 — frozen backbone, FPN3 head | 224×224 (squished) | 0.566 | 0.872 | — |
| E2 — frozen backbone, FPN4 head | 224×224 (squished) | 0.619 | 0.872 | — |
...
*Stored frames are 398×224 (16:9). 224×224 squished the horizontal axis to 0.56×;
320×180 restores the aspect at ~matched pixel budget; 398×224 is native full resolution.
Macro-F1 = mean of per-class F1; Micro-F1 = pooled across classes (higher, dominated by
the frequent easy classes). Temporal numbers are from `best.pt` (best-temporal checkpoint);
E3 spatial 0.790 is from `best_spatial.pt`. Micro-F1 was only measured for E3 (E1/E2/E4/E5
logged Macro only during training); "—" = not recorded, can be backfilled by re-evaluating
the saved checkpoints once the GPU is free.*
